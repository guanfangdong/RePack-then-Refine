...
class Rfft2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # perform FFT using new API
        # torch.rfft(x, signal_ndim=2) -> torch.fft.rfft2(x)
        coeff_complex = torch.fft.rfft2(x)
        
        # Convert complex tensor to the old format (real, imag) in last dimension
        # to maintain compatibility with the rest of the code
        coeff = torch.stack([coeff_complex.real, coeff_complex.imag], dim=-1)
    ...
    def inverse(self, coeff, output_shape):
        ...
        # Convert from old format (real, imag) to complex tensor
        coeff_complex = torch.complex(coeff[..., 0], coeff[..., 1])
        
        # perform iRFFT using new API
        # torch.irfft(coeff, signal_ndim=2, signal_sizes=(self.blocksize,self.blocksize))
        # -> torch.fft.irfft2(coeff_complex, s=(self.blocksize, self.blocksize))
        x = torch.fft.irfft2(coeff_complex, s=(self.blocksize, self.blocksize))
