...
def main(args):
    """
    Run RePack on ImageNet and cache compact latents for DiT training.
    """
    ...
    # Create model:
    tokenizer = RePackTokenizer(args.config, img_size=args.image_size)
    ...
    # Save the remainder shard when fewer than 10,000 images are left.
    if len(latents) > 0:
        latents = torch.cat(latents, dim=0)
        latents_flip = torch.cat(latents_flip, dim=0)
        labels = torch.cat(labels, dim=0)
    ...
    # Calculate and cache latent statistics once extraction is complete.
